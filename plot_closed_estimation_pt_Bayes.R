...
# median.N <- unlist(lapply(bayes.out, FUN = function(x) x$summary$quantiles['N','50%']))
...
for (k0 in 1:length(pts)){
  ...
  Nhats.all <- Nlow.all <- Nhigh.all <- data.frame(k06 = numeric(0),
                                                   k08 = numeric(0),
                                                   k10 = numeric(0),
                                                   k12 = numeric(0),
                                                   k14 = numeric(0),
  ...
  if (length(files.p) == 1){
    ...
    for (k1 in 1:length(ks.txt)){
      for (k2 in 1:length(RMark.out.all[[k1]])){
        ...
        Nlow.all[k2, k1] <- bayes.out[[k2]]$summary$quantiles['N','2.5%']
        Nhigh.all[k2, k1] <- bayes.out[[k2]]$summary$quantiles['N','97.5%']
  ...
  Nlow.all[Nlow.all > 500] <- NA
  Nhigh.all[Nhigh.all > 500] <- NA

  Nhats.all$id <- Nlow.all$id <- Nhigh.all$id <- seq(1, 500, by=1)
  tmp <- na.omit(melt(Nhats.all, id = 'id'))

  tmp.low <- na.omit(melt(Nlow.all, id = 'id'))
  tmp.high <- na.omit(melt(Nhigh.all, id = 'id'))
  ...
  p2 <- ggplot(data = tmp.low,
               aes(x = variable, y = value)) +
    geom_boxplot() +
    scale_x_discrete(name = 'Sample size',
                     labels = c('6', '8', '10', '12', '14', '16')) +
    scale_y_continuous(name = 'Lower 2.5% of Nhat') +
    ggtitle(paste('Capture probability =', ps[k0])) +
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_hline(yintercept = 60) +
    annotate('text',
  ...
  p3 <- ggplot(data = tmp.high,
               aes(x = variable, y = value)) +
    geom_boxplot() +
    scale_x_discrete(name = 'Sample size',
                     labels = c('6', '8', '10', '12', '14', '16')) +
    scale_y_continuous(name = 'Upper 97.5% of Nhat') +
    ggtitle(paste('Capture probability =', ps[k0])) +
    theme(plot.title = element_text(hjust = 0.5)) +
    geom_hline(yintercept = 60) +
    annotate('text',

...
In this study, we focused on estimating abundance only by conducting a short-term intense capture effort from 2 May 2017 to 6 July 2017 so that we could assume that the survival rate of the foraging aggregation is 1.0 and no immigration and emigration was occurring.  
...
dat.size <- read_csv(file = 'data/CMR2018_size_data_2018-06-01.csv') %>%
...
Figure 1. Changes in SCL (cm) of green turtles captured in San Diego Bay from 1990 to psresent. The current reporting period (```r begin.date``` - ```r end.date```) is shown as a shaded region. Those captured within the reporting period are shown in blue. 
...
Body mass (kg) of captured turtles during the reporting period ranged from ```r min(dat.size.new$Mass_kg, na.rm = T)``` to ```r (max(dat.size.new$Mass_kg, na.rm = T))``` (mean = ```r signif(mean(dat.size.new$Mass_kg, na.rm = T), 4)```, SE = ```r signif(SE(dat.size.new$Mass_kg), 2)```, n = ```r length(!is.na(dat.size.new$Mass_kg))```), whereas for the entire dataset it was ```r min(dat.size$Mass_kg, na.rm = T)``` to ```r max(dat.size$Mass_kg, na.rm = T)``` (mean = ```r signif(mean(dat.size$Mass_kg, na.rm = T), 4)```, SE = ```r signif(SE(dat.size$Mass_kg), 2)```, n = ```r length(!is.na(dat.size$Mass_kg))```).
...
Figure 3. Relationship betweenb SCL and Mass. Blue points indicated data from this reporting period (```r begin.date``` - ```r end.date```).  The red point indicates the mass of the turtle converted to kilograms. 
...
dat.size %>% group_by(Turtle_ID) %>% 
  summarise(SCL = first(SCL_cm),
            SCW = first(SCW_cm),
            CCL = first(CCL_cm),
            CCW = first(CCW_cm)) -> dat.size.by.ID

...
#p is variable over all occasions.

p <- lapply(as.list(k), FUN = runif, 0.02, 0.1)
...
for (i in 1:length(k)){
  ...
  save(list = c('summary.data', 'sample.size',
                ...
                'sim.results.all'),
       file = paste0('RData/closed_simulation_pUNIF_k', k[i], '_',
                     run.date, '.RData'))
...
save(list = c('summary.data', 'sample.size',
              'N', 'p', 'k', 'sim.results.data',
              'sim.results.all'),
     file = paste0('RData/closed_simulation_pUNIF_',
                   run.date, '.RData'))
